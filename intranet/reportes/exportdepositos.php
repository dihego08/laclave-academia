...
FROM reservas r INNER JOIN clientes  ON r.id_client=clientes.id_client  where fecha BETWEEN "'.$inicio.'" AND "'.$fin.'"'.' order by r.sed');
/*
  ...
  foreach($rec as $value)
  {
    switch ($i){
        case 7:
            ...
            $rowData .= $value;
            $suma_pos7 = $suma_pos7+$value;
            break;
        case 8:
            $value = '"' . doubleval($value) . '"' . "\t";
            $rowData .= $value;
            $suma_pos8 = $suma_pos8+$value;
            break;
        case 12:
            $value = '"' .doubleval($value) . '"' . "\t";
            $rowData .= $value;
