...
function Calendarize() {
	...
	return {
		...
		buildMonth: function(monthNum, year, opts) {
			...
			$monthNode.classList.add('col-md-12');
			$monthNode.classList.add('mt-3');
			$monthNode.setAttribute('id', "m_"+monthNum);
			$monthNode.setAttribute('hidden', "hidden");

			// Add a Title to the month
			if (opts.showMonth) {
				//$titleNode.innerText = monthNames[monthNum] + (opts.showYear ? " " + year : '');
				$titleNode.innerText = monthNames[monthNum];
			...
			daysInMonth.forEach(function(c, d) {
				...
				$dayNode.setAttribute('id', new Date(c).getFullYear()+"-"+String(parseInt(new Date(c).getMonth() + 1)).padStart(2, "0")+"-"+String(new Date(c).getDate()).padStart(2, "0"));

				$dayNode.setAttribute("data-date", c);
				//$dayNode.innerText = (d + 1);
				$dayNode.innerHTML = '<p>'+(d + 1)+'</p>';
				...
				if (dow === 0) $dayNode.classList.add('weekend');
